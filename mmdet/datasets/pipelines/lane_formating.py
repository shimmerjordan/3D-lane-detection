...
class CollectLanePoints(Collect):
    def __init__(
        ...
        self.lane_extend_pre = lane_extend_pre
    ...
    def target(self, results):
        ...
        for l in range(self.fpn_layer_num):
            ...
            for i, pts in enumerate(gt_points):  # per lane
                ...
                if pts is not None:
                    ...
                    pts = np.int32(pts_f)
                    if pts[None, :, ::-1].min() >= max(output_h, output_w) * -1:
                        # print(pts[None, :, ::-1].min())
            ...
            if torch.min(to_tensor(lane_points_align).float()) != -1.0:
                print('################ mmdet/datasets/pipelines/lane_formating.py ############{}###########################'.format(torch.min(to_tensor(lane_points_align).float())))
        # print(len(gt_points))
        for pts in gt_points:  # per lane
            ...
            if pts is not None and len(pts) > 1:
                ...
                init_point = pts[int(len(pts) * 0.5)]       # init_point
                # pts = pts[:int(len(pts) * 0.5)]
                # start_point, end_point = pts[-1], pts[0]
                start_point, end_point = pts[0], pts[-1]  # ori
                # print("####################{}####################{}####################{}####################".format(start_point, init_point, end_point))

                delta_idx = len(pts) // self.joint_nums
                end_points.append(end_point)
                start_points.append(start_point)

                # init_points.append(init_point)      # init_points

                # joint_nums
                for i in range(self.joint_nums):
                    joint_points.append(pts[i * delta_idx]) # ori
                    total_joint_points.append(pts[i * delta_idx])
                    # joint_points.append(pts[- i * delta_idx - 1])
                ...
                for pt in pts:
                    ...
                    for i in range(self.joint_nums):
                        ...
                        # offset_x = start_points[i][0] - pt[0]
                        # offset_y = start_points[i][1] - pt[1]
        ...
        if len(total_joint_points) > 0:
            for joint_point in total_joint_points:
                draw_umich_gaussian(gt_hm[0], joint_point, radius=self.root_radius)  # start points
