...
def make_mask(shape=(1, 80, 200), device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')):
...
def make_coordmat(shape=(1, 80, 200), device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')):
    x_coord = torch.arange(0, shape[-1], step=1, dtype=torch.float32, device=device)
    x_coord = x_coord.reshape(1, 1, -1)
    # x_coord = np.repeat(x_coord, shape[1], 1)
    x_coord = x_coord.repeat(1, shape[1], 1)
...
class GANetHeadFast(nn.Module):
    ...
    def ktdet_decode(self, heat, offset, error, thr=0.1):
        ...
        def get_virtual_down_coord(coord, offset_map, root_i):
            ...
            for idx, _offset in enumerate(offset_vector):
                # offset_y = _offset[1]  # ori
                offset_y = abs(_offset[1])
                if offset_y < 0:
                    continue
            ...
            # 这里的四舍五入可能是导致 prediction 锯齿的原因之一
        ...
        def get_virtual_root(coord, offset_map):
        ...
        def get_virtual_root_one(coord, offset_map):
        ...
        def _format(heat, offset, error, inds):
            ...
            for y, x, c in zip(inds[0], inds[1], inds[2]):
                ...
                score = heat[y, x, c]
                if offset.shape[-1] == 2:
                    _virtual_root = get_virtual_root_one(coord, offset)
                else:
                    _virtual_root = get_virtual_root(coord, offset)
                _error = error[y, x]
    ...
    def ktdet_decode_fast(self, heat, offset, error, thr=0.1, root_thr=2):
        ...
        mask = torch.lt(offset_split[1], root_thr)  # offset < root_thr
        mask_nms = torch.gt(heat_nms, thr)  # key point score > thr
    ...
    def forward_test(
        ...
        if pts_offset.shape[1] > 2:
            ...
            heat_nms = _nms(cpts_hm)
            offset_split = torch.split(pts_offset, 1, dim=1)
            mask = torch.lt(abs(offset_split[1]), self.root_thr)  # offset < 1
