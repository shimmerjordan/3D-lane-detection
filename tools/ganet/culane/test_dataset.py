...
from tools.ganet.common import COLORS, parse_lanes, convert_coords_formal, parse_anno
from tools.ganet.post_process import PostProcessor
from tools.ganet.lane_metric import LaneMetricCore

# from tools.upload_oss import upload
...
def badCase(f1_measure, precision, recall):
    bad_case = False
    if f1_measure < 0.5 or precision < 0.5 or recall < 0.5:
        bad_case = True
    if precision == 0:
        bad_case = False
    return bad_case
...
def single_gpu_test(seg_model,
    ...
    # TODO: evaluator testing
    eval_width, eval_height, lane_width = 224, 224, 5
    ...
    for i, data in enumerate(data_loader):
        with torch.no_grad():
            ...
            evaluator_single = evaluator.result_single()

            bad_case = badCase(evaluator_single["f1_measure"], 
                evaluator_single["precision"], 
                evaluator_single["recall"])

        if show is not None and show and bad_case:
            ...
            # upload(dst_show_dir, basename)
            # upload(dst_gt_dir, basename + '.gt.jpg')
            # upload(dst_show_vc_dir, basename + '.vc.jpg')
            # upload(dst_show_cc_dir, basename + '.cc.jpg')
